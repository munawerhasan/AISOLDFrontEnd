...
export class StudentDashboardComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.studentService.getStudentDashboard(this.authService.currentUserValue.schoolId, this.authService.currentUserValue.id).subscribe(res => {
      this.dashboardData = res.data;
    })
    if (parseInt(localStorage.getItem('seen')) == 0) {
      this.studentService.getPendingFeeByUserIdAPI(this.authService.currentUserValue.schoolId, this.authService.currentUserValue.id).subscribe((res: any) => {
        this.pendingFeeData = res.data;
        swal({
          title: 'Fee Reminder!',
          text: 'Dear ' + this.authService.currentUserValue.name + ', there is ' + this.pendingFeeData.totalDue.toString() + ' pending amount for the Month of ' + this.pendingFeeData.monthName,
          type: 'warning'
        }).catch(swal.noop);
      });
